...
generally **contested or fuzzy concepts**. Moreover, they are difficult
to observe and isolate as factors that affect democracy in the same way
...
Second, in order to visualize V-Dem responses in a geo-referenced
environment, the operationalized country-specific indicators must be
assigned some measure based on the proportional values recorded in each
country-year. This requires a degree of subjectivity. For example, how
many votes or proportion of votes are necessary to signify "strong"
support for the ruling party (question 15)? Or, which municipalities are
considered part of the "North" (question 6) versus the "West" (question
8)? We privilege the use of granular data that will allow researchers
flexible interpretations of these questions, as well as subsequent
validation from country experts.
...
stronger or weaker democratization in Colombia. - Scoring is based on
the aggregation of response values across all indicators.

# Mapping exercise: 2018 cross-section of Colombia

This section outlines a general pipeline for mapping, following the
conceptualization, measurement, and scoring process above. This exercise
is designed test the viability of mapping cross-sectional country-level
data before expanding the dataset into panel format. Therefore, internal
data from V-Dem is subset to 2018, and the last five subnational
responses are removed because they are not relevant for our study of
Colombia.

For the sake of replication, we can summarize the subnational mapping
procedure into three stages. The first stage is exploratory, identifying
the country and level of analysis for the study. In this case, we
observe that Colombia has fairly complete municipal-level data dating
back to at least the 1950s, at least for a relevant amount of
time-varying variables in V-Dem (e.g., population density, economic
development, voting behavior, etc.).

Second, we collect and format data for mapping purposes. This is a
multi-step labor-intensive process described in more detail below.
Essentially, in this stage, we seek to merge V-Dem data with external
data. For practical purposes, some indicators (e.g., responses 6-9 that
ask to identify 4 cardinal directions) can be grouped together in order
to facilitate data collection. Once grouped, the indicators themselves
are transformed using a cumulative distribution function (pnorm) before
being merged with V-Dem values. This is necessary for capturing the
salience of a particular factor or set of factors for subnational
democracy, according to country experts.
...
sources. In order to facilitate merging tables to shape-files, we use a
key ID (either the official DANE code for each municipality, or the
ArcMap FID number). In the case, we use the `sf` package, so we can
proceed to merge along the DANE code.

[Note]{.underline}: This exercise concentrates on cross-sectional data,
but it will expand to time-series cross-sectional (\~1900-2022).
