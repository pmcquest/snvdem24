...
The survey asks two questions, however, the systematized concept can take many forms. We will create a threshold between urban and rural variables for assigning V-Dem responses. The rurality index from CEDE is a crude measure, but we take the mid-point as a threshold for rural and urban municipalities: if more than half of the population is considered rural inhabitants, then the municipality is considered rural. This can be adjusted later if necessary. 
...
"more" developed before merging with V-Dem data. In the interest of simplicity, we will create a
dichotomous measure for each variable based on median values. 
