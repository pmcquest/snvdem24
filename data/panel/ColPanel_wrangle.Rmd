...
reg <- read_csv("data/panel/Departamentos_y_municipios_de_Colombia_20240204_DANE.csv", 
                col_types = cols(`CODIGO DANE DEL DEPARTAMENTO` = col_number(), 
                                 `CODIGO DANE DEL MUNICIPIO` = col_number()))
colnames(reg)[4] = "CODIGODANE"
 <- ifelse(nchar(CMPD$id) == 4, paste0("0", CMPD$id), CMPD$id)
...
"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")

#library(gptstudio) addin_chatgpt() session\$onSessionEnded(stopApp)

```

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 // this is the
secret key
